"""
This file contains an alternative strategy for trading options on UChicago's proprietary xChange platform
"""
...
def calc_gamma(S, K, T, r, sigma, is_call):
...
def calc_vega(S, K, T, r, sigma, is_call):
...
class OptionBot(UTCBot):
    def __init__(self, *args, **kwargs):
        ...
        self.underlying = 100
        self.placed = False
        self.order_out = {asset: False for asset in CONTRACTS}
    ...
    def update_greeks(self):
        for asset in CONTRACTS[1:]:
            ...
            self.greeks[asset] = [
                ...
                calc_gamma(lcp, strike, time_to_maturity, UTY, u_volatility, is_call),
                calc_theta(lcp, strike, time_to_maturity, UTY, u_volatility, is_call),
                calc_vega(lcp, strike, time_to_maturity, UTY, u_volatility, is_call),
            ]
    ...
    async def handle_order(self):
        if len(self.volatility) > 0 and not all(np.isnan(self.volatility)):
            ...
            bid_amnt = self.params["bid_amnt"]
            ask_amnt = self.params["ask_amnt"]
            ...
            while i < epsilon and i < len(puts_dist) and i < len(calls_dist):
                ...
                if self.position[next_put] < bid_amnt:
                    print(f"Placing BID for {next_put} for {bid_amnt} shares")
                ...
                if all(np.abs(np.array(self.total_greeks())) < np.array(RISK_LIMITS)):
                    print(np.abs(np.array(self.total_greeks())) < np.array(RISK_LIMITS))
                    print(f"Placing BID for {next_call} for {ask_amnt} shares")
            ...
            while i < len(puts_dist) and i < len(calls_dist):
                ...
                if any(np.abs(np.array(self.total_greeks())) > np.array(RISK_LIMITS)):
                    print(
